    ...
    <div class="relative w-full flex flex-col justify-center">
        ...
        <div v-if="isHiddenOptionsDropdown" id="deviceDropdown" class="absolute z-10 divide-y shadow w-full h-full bg-gray-800 overflow-y-auto">
            <ul class="py-1 text-aqua">
                <li v-for="stepResolutionOption in stepResolutionOptions" class="w-full">
                    <button class="w-full block px-4 py-2 hover:bg-aqua1" @click="selectStepResolution(stepResolutionOption)">
                        {{ stepResolutionOption.name }}
                    </button>
                </li>
            </ul>
        </div>
...
<script setup>
    ...
    const { motorStepResolution } = storeToRefs(cmStore)


    const isHiddenOptionsDropdown = ref(false)

    const stepResolutionOptions = ref([
        {
        ...
        {
            name: 'Fourth resolution',
            value: 3
        },
        {
            name: 'Eighth resolution',
            value: 4
        },
        {
            name: 'Sixteenth resolution',
            value: 5
        },
        {
            name: 'Thirty-second resolution',
            value: 6
        }
    ])

    function selectStepResolution(stepResolutionOption){
        motorStepResolution.value = stepResolutionOption
